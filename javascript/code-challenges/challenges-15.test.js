...
let biggerThanLuke = (arr) => {
  ...
  const filteredChars = arr.filter(char => parseInt(char.mass) > lukeMass);


  const names = filteredChars.map(char => char.name);
...
const isSecure = (url) => {
  // Solution code here...
  return url.startsWith("https://") && url.indexOf("//", 6) !== -1;
};
