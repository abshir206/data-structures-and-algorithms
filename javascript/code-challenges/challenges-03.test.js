...
const addTwo = (arr) => {
  // Solution code here...
  var newArr = arr.map(numbers => numbers + 2);
  return newArr;
};
...
const typeNum = (arr) => {
  // Solution code here...
  let newType = arr.filter(numbers => {
...
const containsAnd = (arr) => {
  // Solution code here...
  let newContain = arr.filter(strings => {
    if (strings.includes('and')){
      return strings;
    }
  });
  return newContain;
...
const oddValues = (arr) => {
  // Solution code here...
  var newValues = arr.filter(interger => interger % 2 == 1)
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let newNot = arr.filter((input => !forbiddenValues.includes(input)));
  return newNot;
};
